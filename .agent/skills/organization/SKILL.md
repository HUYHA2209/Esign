...
### Quản trị Tổ chức & Thành viên (Đã hoàn thiện)
Hệ thống cho phép mời (Invite) thành viên khác thông qua email tham gia vào Organization. Các endpoint chính:
- **Invite**: `POST /{orgId}/invitations` (Gửi email kèm token).
- **Xác nhận**: `GET /invitations/verify`, `POST /invitations/{token}/accept`, `POST /invitations/{token}/reject`.
- **Thành viên**: `GET /{orgId}/members`, `PUT /{orgId}/members/{memberId}` (Sửa quyền), `DELETE /{orgId}/members/{memberId}` (Đuổi).
- **Rời / Xóa Tổ chức**: `POST /{orgId}/leave`, `DELETE /{orgId}`.
- **Dashboard**: `GET /{orgId}/dashboard`.
