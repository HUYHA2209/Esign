...
**Important**: Hàm `sendDocumentGroup` cũng đảm nhiệm việc gửi **Thông báo đa kênh** (WebSocket + Email) cho những người nhận thông qua `NotificationsService.sendToUser()`.
...
## Notification Flow (Đã hoàn thiện)
Quá trình gửi Document đã được tích hợp với `NotificationsService`.
- Khi `DocumentService.sendDocumentGroup()` được gọi, nó duyệt qua mảng `signerEmails` và gọi `notificationsService.sendToUser()`.
- API này đẩy realtime WebSocket qua `/queue/notifications` và đồng thời queue một task để gửi Email nhắc nhở.
