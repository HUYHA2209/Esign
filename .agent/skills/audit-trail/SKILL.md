...
### Bước 1: Khởi tạo Service `AuditTrailService`
Cần tạo một Service độc lập (nằm ngoài Document/Signing) chuyên phục vụ log:
```java
@Service
@RequiredArgsConstructor
public class AuditTrailService {
    private final AuditTrailRepository auditTrailRepo;
    private final AuditChainRepository auditChainRepo;
    
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public AuditTrail logEvent(AuditLogRequest request) {
        // 1. Tạo và map dữ liệu cho AuditTrail từ request
        ...
        // 5. Lưu AuditChain
        // 6. Trả về AuditTrail instance
    }
}
```
*(Ghi chú: Dùng `Propagation.REQUIRES_NEW` để đảm bảo lưu vết luôn thành công vào DB dù cho business logic cha có bị lỗi/rollback một số thành phần khác, tránh mất log)*.
