...
import { getUserProfile, updateUserProfile, deleteAccount } from '../../service/userApi';
import { motion } from 'framer-motion';
import { User, Mail, Phone, Camera, Shield, Zap, Lock, FileSignature, AlertTriangle } from 'lucide-react';
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <motion.div
                        ...
                        className="bg-white rounded-[32px] p-10 shadow-premium border border-red-100"
                    >
                        <h3 className="text-xl font-bold text-red-600 font-display mb-2 flex items-center gap-2">
                            <AlertTriangle className="w-6 h-6" /> Khu vực nguy hiểm
                        </h3>
                        <p className="text-secondary-500 text-sm mb-6">
                            Xóa vĩnh viễn tài khoản của bạn và mọi dữ liệu liên quan. Hành động này không thể hoàn tác. Bạn phải rời khỏi tất cả các tổ chức trước khi thực hiện.
                        </p>
                        
                        {!showDeleteConfirm ? (
                            <button 
                                onClick={() => setShowDeleteConfirm(true)}
                                className="px-6 py-3 bg-red-50 text-red-600 font-bold rounded-xl border border-red-100 hover:bg-red-100 hover:text-red-700 transition-colors"
                            >
                                Xóa tài khoản vĩnh viễn
                            </button>
                        ) : (
                            <div className="bg-red-50 p-6 rounded-2xl border border-red-200">
                                <p className="text-sm font-bold text-red-800 mb-3">Xác nhận xóa tài khoản</p>
                                <p className="text-xs text-red-600 mb-4">Vui lòng gõ <strong>XOA TAI KHOAN</strong> vào ô bên dưới để xác nhận (chữ hoa không dấu).</p>
                                <div className="flex gap-4">
                                    <input 
                                        ...
                                        className="flex-1 px-4 py-2 border border-red-200 rounded-xl outline-none focus:ring-2 focus:ring-red-500/20 text-red-900 bg-white"
                                    />
                                    <button 
                                        onClick={() => setShowDeleteConfirm(false)}
                                        className="px-4 py-2 bg-white text-secondary-600 border border-secondary-200 rounded-xl hover:bg-secondary-50 font-bold transition-colors"
                                    >
                                        Hủy
                                    </button>
                                    <button 
                                        ...
                                        disabled={deleting}
                                        className={`px-6 py-2 bg-red-600 text-white rounded-xl font-bold shadow-lg shadow-red-500/30 hover:bg-red-700 transition-all flex items-center gap-2 ${deleting ? 'opacity-70 cursor-not-allowed' : ''}`}
                                    >
